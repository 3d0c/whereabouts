...
func IPManagementKubernetesUpdate(ctx context.Context, mode int, ipam *KubernetesIPAM, ipamConf whereaboutstypes.IPAMConfig) ([]net.IPNet, error) {
	...
	var vmRef string
	if p, err := ipam.Client.GetPod(ipamConf.PodNamespace, ipamConf.PodName); err == nil {
		or := p.GetOwnerReferences()
		for _, entry := range or {
			if entry.Kind != "VirtualMachineInstance" {
				continue
			}
			vmRef = ipam.Config.PodNamespace + "/" + entry.Name
			break
		}
	} else {
		logging.Errorf("Warning. Unable to get pod '%s': %v. Persist IPs for VMs are not going to work", ipamConf.GetPodRef(), err)
	}
	...
	RETRYLOOP:
		for j := 0; j < storage.DatastoreRetries; j++ {
			...
			switch mode {
			case whereaboutstypes.Allocate:
				logging.Debugf("Assigning IP for ipRage: %v, reserverlist: %v, containerID: %v, podref: %v, ifname: %v, vmRef: %v, logicNetworkName: %v, persistIP: %v", ipRange,
					reservelist,
					ipam.ContainerID,
					ipamConf.GetPodRef(),
					ipam.IfName,
