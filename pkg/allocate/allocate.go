...
func AssignIP(
	...
	containerID, podRef, ifName, vmRef, srcPodref, vmNet string, persistIP bool) (
	net.IPNet, []types.IPReservation, error) {
	...
	for i, r := range reservelist {
		if r.VMRef != "" &&
			...
			r.PodRef == srcPodref &&
			r.PersistIP {
			logging.Debugf("Reassign IP: %s vmRef: %q/%s, pod: %s", r.IP.String(), vmRef, vmNet, podRef)
			// if r.PodRef != "" {
